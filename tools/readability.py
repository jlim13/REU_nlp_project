...
t = '/home/john/Desktop/REU/finalassetreports/*.txt'
...
for fileName in glob.glob(t):
    ...
    #print(fileName, Flesch_score(fileName))
    ...
    '''
press_path = '/home/john/Desktop/REU/title_and_urls/*.txt'

for x in glob.glob(press_path):
    with open (x) as fp:
        for line in fp:
...
newsCoefficient = np.corrcoef(Flesh_axis, newscount)[0, 1]
tweetCoefficient = np.corrcoef(Flesh_axis, tweetcount)[0, 1]
print(newsCoefficient, tweetCoefficient)
'''
#print(len(Flesh_axis), len(tweetcount))
plt.plot(Flesh_axis, tweetcount, 'ro')
plt.title('Flesch vs. Twitter Success')
plt.ylabel('Twitter')
plt.xlabel('Flesch score')
plt.axis([0,100,0,35])## x, y
plt.show()
'''
